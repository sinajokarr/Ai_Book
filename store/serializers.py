...
from .models import Category, Customer, Product, CartItem, Comment, Cart, Discount, Address, OrderItem


class CategorySerializer(serializers.ModelSerializer):
    product_count = serializers.IntegerField(read_only=True)
    in_stock_count = serializers.IntegerField(read_only=True)
    max_price = serializers.DecimalField(max_digits=10, decimal_places=2, allow_null=True, read_only=True)
    min_price = serializers.DecimalField(max_digits=10, decimal_places=2, allow_null=True, read_only=True)
    avg_price = serializers.DecimalField(max_digits=10, decimal_places=2, allow_null=True, read_only=True)
...
class ProductSerializer(serializers.ModelSerializer):
    ...
    best_discount_percent = serializers.DecimalField(max_digits=6, decimal_places=2, read_only=True, required=False)
    approved_comments_count = serializers.IntegerField(read_only=True, required=False)
    total_sold = serializers.IntegerField(read_only=True, required=False)

    class Meta:
        model = Product
        fields = [
            "id", "name", "unit_price", "category", "inventory", "slug",
            "tax", "best_discount_percent", "approved_comments_count", "total_sold",
        ]

    def get_tax(self, obj):
        if obj.unit_price is None:
            return None
        return (Decimal(obj.unit_price) * Decimal("0.10")).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
...
class DiscountSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Discount
        fields = ["id", "discount", "description", "label", "application_count", "discounted_price_preview"]
